...
const SpaceFormModal: React.FC<SpaceFormModalProps> = ({ isOpen, onClose, space }) => {
  const queryClient = useQueryClient();
  const isEditMode = !!space;

  const [formData, setFormData] = useState<CreateSpaceDto | UpdateSpaceDto>({
    name: '',
    description: '',
    slug: '',
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateSpaceDto) => adminApi.createSpace(data),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: ({ id, data }: { id: number; data: UpdateSpaceDto }) => adminApi.updateSpace(id, data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['spaces'] });
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    if (errors[name as keyof CreateSpaceDto]) {
  ...
  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof CreateSpaceDto, string>> = {};
