...
const AdminNavItem: FC<AdminNavItemProps> = ({ icon, label, isActive, collapsed, onClick }) => {
  return (
    <li>
      <button
        onClick={onClick}
      ...
      >
        <span className={`${isActive ? 'text-primary' : 'text-gray-500 group-hover:text-gray-700'}`}>{icon}</span>
        {!collapsed && <span className={`ml-3 text-sm tracking-tight ${isActive ? 'text-primary' : ''}`}>{label}</span>}
        {isActive && collapsed && (
